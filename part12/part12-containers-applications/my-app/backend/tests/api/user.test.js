...
describe('user testing', () => {
  test('create new user', async () => {
    ...
    expect(bcrypt.compare(user.password, savedUser.passwordHash)).resolves.toBe(
      true
    );
  });

  test('get all users', async () => {
    const usersInDb = await loadUsersInDB();
    await loadBlogsInDB(usersInDb);
  ...
  describe('attempt to create invalid user fails', () => {
    test('missing fields', async () => {
      ...
      expect(missingAllFieldsResponse.body.error).toEqual(
        'username, name or password missing'
      );
      ...
      expect(missingNameResponse.body.error).toEqual(
        'username, name or password missing'
      );
      ...
      expect(missingUsernameResponse.body.error).toEqual(
        'username, name or password missing'
      );
      ...
      expect(missingPasswordResponse.body.error).toEqual(
        'username, name or password missing'
      );
    });
    ...
    test('fields too short', async () => {
      ...
      expect(passwordTooShortResponse.body.error).toEqual(
        'username and password must be 3 or more characters long'
      );
      ...
      expect(usernameTooShortResponse.body.error).toEqual(
        'username and password must be 3 or more characters long'
      );
    });
  });
